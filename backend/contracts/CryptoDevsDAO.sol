...
contract CryptoDevsDAO is Ownable{
    ...
    modifier activeProposals(uint256 proposalIndex) {
    ...
    function voteOnProposal(uint256 proposalIndex, Vote vote) external nftHolderOnly activeProposals(proposalIndex) {
        ...
        if (vote == Vote.YAY) {
            proposal.yayyVotes += numVotes; 
        } else {
            proposal.nayyVotes += numVotes;
        }
    }
    ...
    inactiveProposalOnly(proposalIndex)
{
    ...
    if (proposal.yayyVotes > proposal.nayyVotes) {
